...
async function resolveFacilityId({ headers = {}, body = {}, requestId, transactionId }) {
  const direct =
    ...
    deepGet(body, [
      ...
      'notification.consentDetail.hiu.id'
    ]);
...
async function getFacility(facilityId, options = {}) {
  ...
  let query = AbdmFacility.findOne({
    active: true,
    $or: [{ 'abdm.hipId': facilityId }, { 'abdm.hiuId': facilityId }, { facilityId }]
  });
...
async function forwardToHospital(facility, path, body, options = {}) {
  const hipId = facility.abdm?.hipId || facility.abdm?.hiuId || facility.facilityId;
  ...
  const targetUrl = await assertSafeOutboundUrl(`${baseUrl}${path}`, {
    label: 'Hospital connector URL',
    allowedHosts: abdmConfig.connectorAllowedHosts,
    requireHttps: process.env.NODE_ENV === 'production',
    allowPrivate: process.env.NODE_ENV !== 'production' && process.env.ABDM_ALLOW_PRIVATE_CONNECTOR_URLS === 'true'
  });
