...
exports.listHiuRequests = async (req, res) => {
  ...
  if (req.query.status) filter.status = req.query.status;
  if (req.query.consentId) filter.consentId = req.query.consentId;
  const requests = await AbdmHiuRequest.find(filter).sort({ createdAt: -1 }).limit(200).lean();
  return res.json({ success: true, requests });
};

exports.getHiuRequest = async (req, res) => {
  if (!isValidObjectId(req.params.requestId)) return res.status(400).json({ success: false, message: 'Invalid HIU request ID' });
  const request = await AbdmHiuRequest.findById(req.params.requestId).lean();
  if (!request) return res.status(404).json({ success: false, message: 'HIU request not found' });
