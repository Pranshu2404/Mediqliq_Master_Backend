...
function summarizeLinkage({ lookup, services, expectedHipId, expectedBridgeId, expectedServiceType = 'HIP' }) {
  ...
  const serviceTypeValid = !type || String(type).toUpperCase().includes(String(expectedServiceType).toUpperCase());
  const activeValid = active === undefined || active === null || active === true || String(active).toLowerCase() === 'true';

  return {
    linked: hipFound && bridgeFound && serviceTypeValid && activeValid,
    hipFound,
    bridgeFound,
    hipTypeValid: serviceTypeValid,
    serviceTypeValid,
    activeValid,
    hipName,
    serviceType: type,
...
exports.createFacility = async (req, res) => {
  try {
    ...
    const actualHiuId = normalizeIdentifier(hiuId || req.body?.abdm?.hiuId);
    ...
    facility.abdm = {
      ...
      hiuId: actualHiuId || facility.abdm?.hiuId,
    ...
    return res.status(existing ? 200 : 201).json({
      ...
      connectorCredentials: credentials
        ? {
            ...
            hiuId: facility.abdm?.hiuId || null,
...
exports.updateFacility = async (req, res) => {
  try {
    ...
    if (req.body.hipId !== undefined) facility.abdm.hipId = normalizeIdentifier(req.body.hipId);
    if (req.body.hiuId !== undefined) facility.abdm.hiuId = normalizeIdentifier(req.body.hiuId);
    if (req.body.hipName !== undefined) facility.abdm.hipName = normalizeIdentifier(req.body.hipName);
    if (req.body.hiuName !== undefined) facility.abdm.hiuName = normalizeIdentifier(req.body.hiuName);
...
exports.verifyFacilityLinkage = async (req, res) => {
  try {
    const facility = await findFacility(req.params.facilityId);
    if (!facility) return res.status(404).json({ error: 'Facility not found' });
    const expectedHipId = normalizeIdentifier(req.body.hipId || facility.abdm?.hipId);
    const expectedHiuId = normalizeIdentifier(req.body.hiuId || facility.abdm?.hiuId);
    const expectedBridgeId = normalizeIdentifier(req.body.bridgeId || facility.abdm?.bridgeId || abdmConfig.bridgeId);
    if (!expectedHipId || !expectedBridgeId) return res.status(400).json({ error: 'HIP ID and bridge ID are required' });
    if (facility.services?.hiu && !expectedHiuId) return res.status(400).json({ error: 'HIU ID is required because HIU service is enabled' });
    facility.abdm.linkageCheckedAt = new Date();
    try {
      const services = await getBridgeServices();
      const hipLookup = await getBridgeByServiceId(expectedHipId);
      const hipSummary = summarizeLinkage({ lookup: hipLookup, services, expectedHipId, expectedBridgeId, expectedServiceType: 'HIP' });
      let hiuLookup = null;
      let hiuSummary = null;
      if (facility.services?.hiu) {
        hiuLookup = await getBridgeByServiceId(expectedHiuId);
        hiuSummary = summarizeLinkage({ lookup: hiuLookup, services, expectedHipId: expectedHiuId, expectedBridgeId, expectedServiceType: 'HIU' });
      }
      const linked = hipSummary.linked && (!hiuSummary || hiuSummary.linked);
      facility.abdm.hipId = expectedHipId;
      facility.abdm.hiuId = expectedHiuId || facility.abdm.hiuId;
      facility.abdm.bridgeId = expectedBridgeId;
      facility.abdm.verificationResponse = {
        hip: { summary: hipSummary, lookup: hipLookup },
        hiu: hiuSummary
          ? { summary: hiuSummary, lookup: hiuLookup }
          : undefined,
        services
      };
      facility.abdm.linkageStatus = linked ? 'LINKED' : 'FAILED';
      facility.abdm.active = linked;
      if (linked) {
        facility.abdm.linkageVerifiedAt = new Date();
        facility.abdm.linkageVerifiedBy = actor(req);
        facility.abdm.hipName = hipSummary.hipName || facility.abdm.hipName;
        facility.abdm.hiuName = hiuSummary?.hipName || facility.abdm.hiuName;
      ...
      return res.status(linked ? 200 : 409).json({
        success: linked,
        linked,
        expected: {
          hipId: expectedHipId,
          hiuId: expectedHiuId || null,
      ...
      facility.abdm.linkageStatus = 'FAILED';
      facility.abdm.active = false;
      facility.abdm.verificationResponse = {
        error: error.message,
        checkedAt: new Date()
      };
...
exports.retryJob = async (req, res) => {
  ...
  const job = await AbdmJob.findById(req.params.jobId);
...
exports.replayWebhookEvent = async (req, res) => {
  const AbdmJob = require('../models/AbdmJob');
  const event = await AbdmWebhookEvent.findById(req.params.eventId).select('+payload');
  if (!event) return res.status(404).json({ error: 'Webhook event not found' });
  if (!event.payload) {
    return res.status(409).json({
      error: 'Payload retention is disabled; this event cannot be replayed'
    });
  }
