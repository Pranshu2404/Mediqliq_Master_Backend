...
function authorize(req) {
  if (!SERVICE_TOKEN) {
    if (process.env.NODE_ENV === 'production') throw appError('MEDIQLIQ_SERVICE_TOKEN is required in production', 503, 'SERVICE_TOKEN_MISSING');
    return;
  }
...
function tenantIdentity(req) {
  const tenantCode = String(req.headers['x-mediqliq-tenant-code'] || '').trim().toUpperCase();
  const facilityId = String(req.headers['x-mediqliq-facility-id'] || '').trim();
  if (!tenantCode || !facilityId) {
    throw appError('Master tenant/facility context is required', 401, 'TENANT_CONTEXT_REQUIRED');
  }
  return { tenantCode, facilityId };
}
...
function handleAad(tenant) {
  return Buffer.from(`mediqliq-abdm-key-handle:${KEY_VERSION}:${tenant.tenantCode}:${tenant.facilityId}`);
}

function encodeHandle(material, tenant) {
  const iv = crypto.randomBytes(12);
  const cipher = crypto.createCipheriv('aes-256-gcm', secretKey(), iv);
  cipher.setAAD(handleAad(tenant));
...
function decodeHandle(handle, tenant) {
  ...
  try {
    ...
    decipher.setAAD(handleAad(tenant));
...
async function receiverKeyMaterial(body, tenant) {
  ...
  return {
    ...
    keyHandle: encodeHandle(privateMaterial, tenant)
  };
}
...
async function decrypt(body, tenant) {
  if (process.env.NODE_ENV === 'production' && body.privateMaterial) {
    throw appError('Production decryption accepts only opaque keyHandle receiver material', 422, 'PRIVATE_MATERIAL_FORBIDDEN');
  }
  if (process.env.NODE_ENV === 'production' && !body.keyHandle) {
    throw appError('Production decryption requires an opaque keyHandle', 422, 'KEY_HANDLE_REQUIRED');
  }
  const material = body.keyHandle ? decodeHandle(body.keyHandle, tenant) : body.privateMaterial;
  if (material?.tenantCode && material.tenantCode !== tenant.tenantCode) {
    throw appError('Key handle does not belong to this tenant', 403, 'KEY_HANDLE_TENANT_MISMATCH');
  }
  if (material?.facilityId && material.facilityId !== tenant.facilityId) {
    throw appError('Key handle does not belong to this facility', 403, 'KEY_HANDLE_FACILITY_MISMATCH');
  }
...
async function route(req, res) {
  try {
    ...
    const tenant = tenantIdentity(req);
    const body = await readJson(req);
    if (req.url === '/v1/receiver-key-material') return send(res, 200, await receiverKeyMaterial(body, tenant));
    if (req.url === '/v1/encrypt') return send(res, 200, await encrypt(body));
    if (req.url === '/v1/decrypt') return send(res, 200, await decrypt(body, tenant));
