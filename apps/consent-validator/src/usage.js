...
async function releaseExpiredReservations(tenantKey) {
  ...
  for (const reservation of expired) {
    ...
    await releaseReservation(reservation.reservationId, tenantKey, 'EXPIRED');
  }
}

async function reserveUsage({ tenantKey, consentIdHash, operationType, operationHash, frequency }) {
  if (!tenantKey) throw new Error('tenantKey is required for consent usage');
  if (!frequency?.maxUses) return null;
  await releaseExpiredReservations(tenantKey);
  const existing = await ConsentUsageReservation.findOne({ tenantKey, operationHash });
  ...
  return withOptionalTransaction(async (session) => {
    ...
    const previous = await ConsentUsageReservation.findOne({ tenantKey, operationHash }, null, queryOptions);
    if (previous?.status === 'COMMITTED' || previous?.status === 'RESERVED') return previous;

    let window = await ConsentUsageWindow.findOne(
      { tenantKey, consentIdHash, operationType, windowStart: bounds.start },
      null,
      queryOptions
    );
    if (!window) {
...
async function commitReservation(reservationId, tenantKey) {
  return withOptionalTransaction(async (session) => {
    const queryOptions = session ? { session } : {};
    const reservation = await ConsentUsageReservation.findOne({ reservationId, tenantKey }, null, queryOptions);
    if (!reservation) {
    ...
    await ConsentUsageWindow.updateOne(
      { _id: reservation.windowId, tenantKey, reserved: { $gte: 1 } },
...
async function releaseReservation(reservationId, tenantKey, status = 'RELEASED') {
  return withOptionalTransaction(async (session) => {
    const queryOptions = session ? { session } : {};
    const reservation = await ConsentUsageReservation.findOne({ reservationId, tenantKey }, null, queryOptions);
