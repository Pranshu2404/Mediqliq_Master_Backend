...
const apiLimiter = rateLimit({
  ...
  legacyHeaders: false,
  keyGenerator: (req) => sha256(`${String(req.headers['x-mediqliq-tenant-code'] || 'unknown')}:${String(req.headers['x-mediqliq-facility-id'] || 'unknown')}:${String(req.headers[config.serviceTokenHeader] || req.headers.authorization || 'service')}`),
  message: {
    valid: false,
    decision: 'DENY',
...
function tenantContext(req) {
  ...
  if (!tenantCode || !facilityId) {
    const error = new Error('Master tenant/facility context is required');
  ...
  return {
    tenantCode,
    facilityId,
    tenantKey: sha256(`${tenantCode}:${facilityId}`)
  };
}
...
app.post('/v1/validate', async (req, res, next) => {
  try {
    const result = await validateConsent(req.body || {}, tenantContext(req));
...
app.post('/v1/status-events', async (req, res, next) => {
  try {
    return res.status(202).json(await recordStatusEvent(req.body || {}, tenantContext(req)));
  } catch (error) {
    return next(error);
  }
});
...
app.post('/v1/usage/:reservationId/commit', async (req, res, next) => {
  try {
    const reservation = await commitReservation(req.params.reservationId, tenantContext(req).tenantKey);
    return res.json({
      success: true,
...
app.post('/v1/usage/:reservationId/release', async (req, res, next) => {
  try {
    const reservation = await releaseReservation(req.params.reservationId, tenantContext(req).tenantKey);
    return res.json({
      success: true,
