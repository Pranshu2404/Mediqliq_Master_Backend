...
const consentStatusSchema = new mongoose.Schema(
  {
    ...
    sourceEventIdHash: { type: String, required: true, index: true },
    source: { type: String, default: 'HOSPITAL_BACKEND' },
    metadata: mongoose.Schema.Types.Mixed
  },
  { timestamps: true }
);
consentStatusSchema.index({ tenantKey: 1, consentIdHash: 1, effectiveAt: -1 });
consentStatusSchema.index({ tenantKey: 1, sourceEventIdHash: 1 }, { unique: true });
...
usageWindowSchema.index(
  { tenantKey: 1, consentIdHash: 1, operationType: 1, windowStart: 1 },
  { unique: true }
);
...
usageReservationSchema.index({ tenantKey: 1, operationHash: 1 }, { unique: true });
...
decisionSchema.index({ tenantKey: 1, operationHash: 1 });
